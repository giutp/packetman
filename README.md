...
Principais desafios enfrentados ao longo do desenvolvimento:

- **Comunicação entre máquinas via cabo Ethernet** — Entender como estabelecer comunicação direta usando raw sockets na camada de enlace (AF_PACKET), sem a abstração do IP/TCP, foi o desafio inicial mais significativo do trabalho.
- **Debug de dois processos simultâneos** — Com cliente e servidor rodando em máquinas separadas, depurar o estado de ambos ao mesmo tempo foi trabalhoso; requereu o uso extensivo de logs e mensagens de diagnóstico.
- **Montagem dos buffers e serialização bit a bit** — Construir e interpretar os frames do protocolo Kermit bit a bit (campos de 5 e 6 bits empacotados em structs com `__attribute__((packed))`) exigiu cuidado redobrado; a versão inicial usava operações bitwise manuais antes de migrar para as diretivas de campo de bits do C.
- **Uso do ncurses** — Aprender a API do ncurses para criar e atualizar janelas de terminal de forma não-bloqueante foi uma curva de aprendizado à parte, especialmente para sincronizar a renderização com a chegada de pacotes.
